...
sub examine {
	...
	if ( /^$args{id}\s+(\d+)\s+(RW|RO|BK)\s+(\d+)\s+K/ ) {
	    ...
	    if ( /^$args{id}\s+(\d+)\s+(RW|RO|BK)\s+(\d+)\s+K\s+([\w-]+)/ ) {
...
sub dump {
    ...
    if ( $gzip && $bzip2 ) {
	$self->_Carp("Invalid argument combination: only one of 'gzip' or 'bzip2' may be specified");
	return;
    }
	...
	if ( $file =~ /\.gz$/ && not defined $gzip ) {
	    $gzip = $gzip_default;
	} elsif ( $file =~ /\.bz2$/ && not defined $bzip2 ) {
	    $bzip2 = $bzip2_default;
	}
...
sub restore {
    ...
    if ( $gunzip && $bunzip2 ) {
	$self->_Carp("Invalid argument combination: only one of 'gunzip' or 'bunzip2' may be specified");
	return;
    }
	...
	if ( $file =~ /\.gz$/ && not defined $gunzip ) {
	    $gunzip = 1;
	} elsif ( $file =~ /\.bz2$/ && not defined $bunzip2 ) {
	    $bunzip2 = 1;
	}
