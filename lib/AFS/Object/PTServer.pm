#
# $Id: PTServer.pm,v 4.1 2003/10/28 21:09:55 wpm Exp $
#
...
sub getGroupByName {
    ...
    my $name = shift;
    return unless ref $self->{_groups} && ref $self->{_groups}->{_byName};
    return $self->{_groups}->{_byName}->{$name};
}
...
sub getGroup {
    ...
    if ( exists $args{name} ) {
	return unless ref $self->{_groups} && ref $self->{_groups}->{_byName};
	return $self->{_groups}->{_byName}->{$args{name}};
    }
...
sub getUserByName {
    ...
    my $name = shift;
    return unless ref $self->{_users} && ref $self->{_users}->{_byName};
    return $self->{_users}->{_byName}->{$name};
}
...
sub getUser {
    ...
    if ( exists $args{name} ) {
	return unless ref $self->{_users} && ref $self->{_users}->{_byName};
	return $self->{_users}->{_byName}->{$args{name}};
    }
